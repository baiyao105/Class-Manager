...
class Class(BaseModel, ArchiveMixin, OrderMixin, table=True):
    """班级数据模型
    ...
    teacher_contact: Optional[str] = Field(
        default=None,
        description="班主任联系方式",
        max_length=100
    )
    ...
    max_students: int = Field(
        default=ClassConstants.MAX_CLASS_SIZE,
        description="最大学生数量"
    )
    ...
    def validate_max_students(cls, v):
        ...
        if not (ClassConstants.MIN_CLASS_SIZE <= v <= ClassConstants.MAX_CLASS_SIZE):
            raise ValueError(
                f'班级最大人数必须在{ClassConstants.MIN_CLASS_SIZE}-{ClassConstants.MAX_CLASS_SIZE}范围内'
            )
        return v
    ...
    def add_student(self, student: "Student") -> bool:
        ...
        # 检查学号是否重复
        if any(s.student_number == student.student_number 
               for s in self.students if not s.is_deleted):
            return False
...
class Group(BaseModel, ArchiveMixin, OrderMixin, table=True):
    """小组数据模型"""
    
    __tablename__ = "cm_groups"
    ...
    members: List["Student"] = Relationship(
        ...
        sa_relationship_kwargs={"lazy": "select", "foreign_keys": "[Student.group_id]"}
    )
...
class ClassCreate(SQLModel):
    ...
    class_type: ClassType = Field(default=ClassType.REGULAR, description="班级类型")
...
class ClassUpdate(SQLModel):
    ...
    teacher_contact: Optional[str] = Field(default=None, description="班主任联系方式", max_length=100)
    class_type: Optional[ClassType] = Field(default=None, description="班级类型")

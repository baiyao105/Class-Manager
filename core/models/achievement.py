...
class AchievementTemplate(BaseModel, ArchiveMixin, OrderMixin, table=True):
    ...
    condition_expression: str = Field(description="条件表达式(Python表达式)", sa_column=Column(Text))
    ...
    achievements: list["Achievement"] = Relationship(
        back_populates="template", sa_relationship_kwargs={"lazy": "select", "cascade": "all, delete-orphan"}
    )
    ...
    def can_be_awarded_to(self, student: "Student") -> bool:
        ...
        if not self.is_repeatable:
            # 检查学生是否已经获得过这个成就
            existing_count = len([a for a in student.achievements if a.template_id == self.id and not a.is_deleted])
            if existing_count > 0:
                return False
...
class Achievement(BaseModel, ArchiveMixin, table=True):
    ...
    template: AchievementTemplate = Relationship(
        back_populates="achievements", sa_relationship_kwargs={"lazy": "select"}
    )
    student: "Student" = Relationship(back_populates="achievements", sa_relationship_kwargs={"lazy": "select"})
...
class ScoreModificationTemplate(BaseModel, ArchiveMixin, OrderMixin, table=True):
    ...
    # 模板设置
    is_active: bool = Field(default=True, description="是否启用")
    is_visible: bool = Field(default=True, description="是否在界面中显示")

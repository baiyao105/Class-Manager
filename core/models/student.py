...
class Student(BaseModel, ArchiveMixin, OrderMixin, table=True):
    ...
    group_id: Optional[int] = Field(
        default=None,
        foreign_key="cm_groups.id",
        description="所属小组ID"
    )
    
    # 分数信息
    current_score: float = Field(
        default=StudentConstants.DEFAULT_SCORE,
        description="当前分数"
    )
    total_score: float = Field(
        default=StudentConstants.DEFAULT_SCORE,
        description="总分（包含历史分数）"
    )
    ...
    # 分数变化时间记录
    highest_score_time: Optional[datetime] = Field(
        default=None,
        description="最高分达成时间"
    )
    lowest_score_time: Optional[datetime] = Field(
        default=None,
        description="最低分产生时间"
    )
    
    # 重置相关
    last_reset_time: Optional[datetime] = Field(
        default=None,
        description="上次重置时间"
    )
    ...
    def validate_scores(cls, v):
        ...
        if not (StudentConstants.MIN_SCORE <= v <= StudentConstants.MAX_SCORE):
            raise ValueError(
                f'分数必须在{StudentConstants.MIN_SCORE}-{StudentConstants.MAX_SCORE}范围内'
            )
        return v
    ...
    def add_score(self, score_change: float, reason: str = "") -> float:
        ...
        # 限制分数范围
        new_score = max(StudentConstants.MIN_SCORE, 
                       min(StudentConstants.MAX_SCORE, new_score))
    ...
    def get_rank_in_class(self) -> Optional[int]:
        ...
        # 获取班级中所有活跃学生，按分数降序排列
        active_students = [
            s for s in self.class_.students 
            if s.status == StudentStatus.ACTIVE and not s.is_deleted
        ]

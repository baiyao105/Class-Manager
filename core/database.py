...
class DatabaseManager:
    ...
    def create_sample_data(self):
        """创建示例数据"""
        from .services import ClassService, StudentService, AchievementService
        ...
        with Session(self.engine) as session:
            ...
            # 创建示例成就
            achievements = [
                achievement_service.create_achievement(students[0].id, "数学竞赛一等奖", "在全国数学竞赛中获得一等奖", 100),
                achievement_service.create_achievement(students[0].id, "优秀学生干部", "担任班长期间表现优秀", 50),
                achievement_service.create_achievement(students[1].id, "英语演讲比赛冠军", "在校英语演讲比赛中获得冠军", 80),
                achievement_service.create_achievement(students[2].id, "文学创作奖", "在校文学创作比赛中获奖", 60),
            ]
            
            print(f"✅ 示例数据创建完成：{len(class_service.get_all_classes())}个班级，{len(student_service.get_all_students())}名学生，{len(achievement_service.get_all_achievements())}项成就")

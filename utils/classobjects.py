    ...
    def reset_all_defaults(self):
        ...
        for c in copy.deepcopy(self.classes).keys():  # 重置班级信息
            if c in DEFAULT_CLASSES:
                self.classes[c].cleaning_mapping = DEFAULT_CLASSES[
                    c
                ].cleaning_mapping  # 恢复默认卫生打扫规则
                self.classes[c].homework_rules = DEFAULT_CLASSES[
                    c
                ].homework_rules  # 恢复作业规则
                ...
                for s in copy.deepcopy(
                    self.classes[c].students
                ).keys():  # 重置这个班级的学生信息
                    if s in default_students:
                        self.classes[c].students[s].name = default_students[
                            s
                        ].name  # 恢复学生姓名
                        self.classes[c].students[s].num = default_students[
                            s
                        ].num  # 恢复学号
        ...
        *,
        ...
        if key in self.modify_templates.keys():
            ...
            if self.modify_templates[key].cant_replace:
                ...
                if not strict:
                    return (
                        ClassObj.EditErrInfo.AddTemplate.ReplacingUnreplaceableTemplate
                    )
        ...
        for m in modify:
            ...
            if not success:
                ...
                for m in succeed:
                    m.retract()
                QMessageBox.warning(
                    "向学生" + m.target.name + "发送点评出现错误，本组点评已撤回"
                )
                Base.log(
                    "I",
                    f"---------------------\n发送失败，总数:{len(modify)}",

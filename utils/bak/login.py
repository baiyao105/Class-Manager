...
def login():
    ...
    aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaawaaaaaaaaaaaaaaaaaaa = (
        2**63
    )
    aaaaa = random.randint(
        1,
        aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaawaaaaaaaaaaaaaaaaaaa,
    )
    # 初始化窗口并记录日志
    ...
    ctk.CTkLabel(window, text="你好！").grid(
        row=0, column=1, padx=(75, 0), pady=(10, 0)
    )
    ctk.CTkLabel(window, text="用户名:").grid(
    ...
    def usr_login():
        ...
        if usr_name in usrs_info:
            ...
            if usr_pwd == str(usrs_info[str(usr_name)]):
                ...
                if usr_name in recently_signed_in:
                    tkinter.messagebox.showinfo(
                        title="你好", message="很高兴见到你，" + usr_name + "！"
                    )
                else:
                    tkinter.messagebox.showinfo(
                        title="你好", message="欢迎回来，" + usr_name + "！"
                    )
                window.destroy()
            else:
            ...
            is_sign_up = tkinter.messagebox.askyesno(
                "提示 ", "当前用户名还没有注册，现在注册？"
            )
            # 提示需不需要注册新用户
            if is_sign_up:
    ...
    def usr_sign_up():
        ...
        ctk.CTkLabel(window_sign_up, text="用户名: ").grid(
        ...
        ctk.CTkLabel(window_sign_up, text="密码: ").grid(
            row=1, column=0, padx=(10, 0), pady=(10, 0)
        )
        ...
        ctk.CTkLabel(window_sign_up, text="再次输入密码: ").grid(
            row=2, column=0, padx=(10, 0), pady=(10, 0)
        )
        entry_usr_pwd_confirm = ctk.CTkEntry(
            window_sign_up, textvariable=new_pwd_confirm, show="*"
        )
        entry_usr_pwd_confirm.grid(
            row=2, column=1, columnspan=2, padx=(10, 0), pady=(10, 0)
        )
        btn_comfirm_sign_up = ctk.CTkButton(
            window_sign_up, text="注册", command=sign, width=8
        )
        btn_comfirm_sign_up.grid(
            row=4, column=0, columnspan=2, padx=(50, 0), pady=(10, 0)
        )

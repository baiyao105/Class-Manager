...
class Message:
    ...
    @overload
    def __init__(self, content: str, edit_time: float, sender: str, others: dict):
        """创建一个新的信息对象。
    ...
    def to_string(self):
        "把这个Message转成一个字符串"
        return json.dumps(
            {"content": self.content, "edit_time": self.edit_time, "sender": self.sender, "others": self.others}
        )

    def load_from(self, obj: str | bytes | dict):
...
class DataPack:
    ...
    def __init__(self, type: str = "default", devinfo: DevInfo | None = None, data: DataType = None):
        self.data = data
...
class Connection:
    ...
    def send_rawdata_to(self, data: bytes | str, addr: str, port: int, timeout: float = -1) -> None:
        """发送原始数据到指定的地址
        ...
        while 1:
            try:
                Base.log(
                    "D",
                    f"向{'[' if connection_mode == 'ipv6' else ''}{addr}{']' if connection_mode == 'ipv6' else ''}:{port}发送数据：{data}",
                    "Connection.send_rawdata_to",
                )
                ...
                Base.log(
                    "D",
                    f"向{'[' if connection_mode == 'ipv6' else ''}{addr}{']' if connection_mode == 'ipv6' else ''}:{port}发送数据成功",
                    "Connection.send_rawdata_to",
                )
                sock.close()
                return
    ...
    def recv_rawdata_as(
        self, addr: str, port: int, timeout: float = -1, return_addr: bool = False
        ...
        while 1:
            try:
                Base.log(
                    "D",
                    f"在{'[' if connection_mode == 'ipv6' else ''}{addr}{']' if connection_mode == 'ipv6' else ''}:{port}等待数据",
                    "Connection.recv_rawdata_as",
                )
                sock = socket.socket(socket_type, socket.SOCK_STREAM)
                if timeout >= 0:
                    sock.settimeout(timeout)
    ...
    @overload
    def send_datapack_to(
        self,
        datapack: DataPack,
        addr: str,
        port: int,
        timeout: float = -1,
        errors: Literal["ignore", "raise", "log"] = "raise",
    ) -> bool:
    ...
    def send_datapack_to(
        ...
        arg1: DataPack | str,
        arg2: str | DataType,
        arg3: int | DevInfo | None,
        arg4: float | str | None = None,
        arg5: Literal["ignore", "raise", "log"] | int = None,
        arg6: float | None = None,
        arg7: Literal["ignore", "raise", "log"] = None,
    ) -> bool:
        ...
        try:
            ...
            if errors == "log":
                Base.log_exc(
                    "E",
                    f"向{'[' if connection_mode == 'ipv6' else ''}{addr}{']' if connection_mode == 'ipv6' else ''}发送数据失败",
                    "Connection.send_datapack_to",
                    "W",
                )
            return False

    def recv_datapack_as(
        self, addr: str, port: int, timeout: float = -1, return_addr: bool = False
    ) -> DataPack | tuple[DataPack, Any]:
        """等待DataPack
    ...
    def send_raw(
        ...
        if errors == "raise":
            self.send_rawdata_to(data, self.target_addr, self.target_port, timeout if timeout else self.timeout)
            return True
        try:
            self.send_rawdata_to(data, self.target_addr, self.target_port, timeout if timeout else self.timeout)
            return True
    ...
    def recv_raw(
        ...
        "接收原始数据（快捷方式）"
        if errors == "raise":
            return self.recv_rawdata_as(
                self.self_addr, self.self_port, timeout if timeout else self.timeout, return_addr=return_addr
            )
        try:
            return self.recv_rawdata_as(
                self.self_addr, self.self_port, timeout if timeout else self.timeout, return_addr=return_addr
    ...
    @overload
    def send_datapack(
        self, datapack: DataPack, errors: Literal["ignore", "raise", "log"] = "raise", timeout: float | None = None
    ) -> bool:
        "发送DataPack"
    ...
    def send_datapack(
        self,
        arg1: DataPack | str,
        arg2: Literal["ignore", "raise", "log"] | DataType = None,
        arg3: float | None | Literal["ignore", "raise", "log"] = None,
        arg4: float | None = None,
    ) -> bool:
        "发送DataPack"
        if isinstance(arg1, DataPack):
            return self.send_datapack_to(
                arg1,
                self.target_addr,
                self.target_port,
        ...
        if isinstance(arg1, str):
            return self.send_datapack_to(
                DataPack(arg1, self.devinfo, arg2),
                self.target_addr,
                self.target_port,
                arg4 if arg4 else self.timeout,
...
class Server(Connection):
    ...
    def __init__(self, addr: str, port: int, max_conn: int = 10):
        ...
        super().__init__(
            ...
            None,
            "Server",
            DevInfo("Server", addr, port, socket.gethostname(), sys.version_info, {}),
            5,
        )
    ...
    def get_client(self, obj: DataPack | DevInfo) -> ProcessingClient:
        "获取一个客户端"
    ...
    def send_to_client(
        ...
        client: ProcessingClient,
        arg1: DataPack | str,
        arg2: float | None | DataType,
        arg3: Literal["ignore", "raise", "log"] | float | None = None,
        arg4: Literal["ignore", "raise", "log"] = None,
    ) -> bool:
        if isinstance(arg1, DataPack):
            return self.send_datapack_to(
                arg1, client.addr, client.port, arg2 if arg2 else self.timeout, arg3 if arg3 else "raise"
            )
        if isinstance(arg1, str):
            return self.send_datapack_to(
                arg1,
    ...
    def wait_for_client_connection(self):
        ...
        while True:
            ...
            Base.log(
                "I",
                f"收到一个客户端连接请求，来自{'[' if connection_mode == 'ipv6' else ''}{req.devinfo.addr}{']' if connection_mode == 'ipv6' else ''}:{req.devinfo.port}",
                "Connection.Server.wait_for_client_connection",
            )
            ...
            def handle_client():
                ...
                try:
                    ...
                    if len(self.processing_clients) < self.max_conn:
                        ...
                        Base.log(
                            "I",
                            f"将{client.addr}:{client.port}添加到处理客户端列表, 当前连接数：{len(self.processing_clients)}",
                            "Connection.Server.handle_client",
                        )
                    else:
                        Base.log("I", "客户端连接失败，服务器已满", "Connection.Server.handle_client")
                        self.send_to_client(
                            client, SocketMsg.Connection.ServerError, SocketMsg.Connection.ServerErrorInfo.Full
                        )

                except TimeoutError:
                    Base.log("W", "客户端连接超时，关闭连接", "Connection.Server.handle_client")
                    self.send_to_client(
                        client, SocketMsg.Connection.ServerError, SocketMsg.Connection.ServerErrorInfo.TimeOut
                    )
    ...
    def get_request(
        self, type: str | Iterable[str], timeout: float = -1, from_client: ProcessingClient = None
    ) -> DataPack:
                ...
                if isinstance(type, str):
                    ...
                    for t in type:
                        if request.type == t:
                            ...
                            return request
            time.sleep(0.01)
        raise TimeoutError(f"在{timeout!r}秒内没有获取到{type!r}类型的请求")
    ...
    def check_client(self, client: ProcessingClient) -> bool:
        ...
        for i in range(max_retry):
            Base.log(
                "I",
                f"检查{'[' if connection_mode == 'ipv6' else ''}{client.addr}{']' if connection_mode == 'ipv6' else ''}:{client.port}的连接",
            ...
            try:
                ...
                Base.log(
                    "I",
                    f"向{'[' if connection_mode == 'ipv6' else ''}{client.addr}{']' if connection_mode == 'ipv6' else ''}:{client.port}发送ServerKeepAliveCheck",
                    "Connection.Server.keep_alive_check",
                )
                req = self.get_request(SocketMsg.Connection.ServerKeepAliveCheckReply, 1, client)
                Base.log(
                    "I",
                    f"收到{'[' if connection_mode == 'ipv6' else ''}{client.addr}{']' if connection_mode == 'ipv6' else ''}:{client.port}的ServerKeepAliveCheckReply，连接继续（内容：{req.data!r}）",
                    "Connection.Server.keep_alive_check",
                )
                is_alive = True
                return True
                ...
                Base.log(
                    "W",
                    f"在1秒内没有收到{'[' if connection_mode == 'ipv6' else ''}{client.addr}{']' if connection_mode == 'ipv6' else ''}:{client.port}的ServerKeepAliveCheckReply... ({i + 1} / {max_retry})",
                    "Connection.Server.keep_alive_check",
                )
        if not is_alive:
            Base.log(
                "W",
                f"{'[' if connection_mode == 'ipv6' else ''}{client.addr}{']' if connection_mode == 'ipv6' else ''}:{client.port}的连接已断开，发送ServerDisconnect",
                "Connection.Server.keep_alive_check",
            )
            try:
                ...
                Base.log(
                    "W",
                    f"{'[' if connection_mode == 'ipv6' else ''}{client.addr}:{client.port}{']' if connection_mode == 'ipv6' else ''}连ServerDisconnect也没接，多半似掉了",
                    "Connection.Server.keep_alive_check",
                )
            Base.log(
                "W",
                f"{'[' if connection_mode == 'ipv6' else ''}{client.addr}{']' if connection_mode == 'ipv6' else ''}:{client.port}的连接已断开",
                "Connection.Server.keep_alive_check",
            )
    ...
    def keep_alive_check(self):
        ...
        while True:
            ...
            for client in self.processing_clients:
                Thread(
                    name=f"KeepAliveCheck({'[' if connection_mode == 'ipv6' else ''}{client.addr}{']' if connection_mode == 'ipv6' else ''}:{client.port}))",
    ...
    def wait_for_client_keepalive_check(self):
        ...
        while True:
            try:
                ...
                Base.log(
                    "I",
                    f"收到{'[' if connection_mode == 'ipv6' else ''}{client.addr}{']' if connection_mode == 'ipv6' else ''}:{client.port}的ClientKeepAliveCheck，发送ClientKeepAliiveCheckReply",
                    "Server.wait_for_client_keepalive_check",
                )
                ...
                def send(client: ProcessingClient):
                    ...
                    for i in range(retry):
                        try:
                            ...
                            Base.log_exc_short(
                                f"向{'[' if connection_mode == 'ipv6' else ''}{client.addr}{']' if connection_mode == 'ipv6' else ''}:{client.port}"
                                f"发送ClientKeepAliveCheckReply失败（{i + 1} / {retry}）",
                                "Server.wait_for_client_keepalive_check",
                            )
                    Base.log(
                        "W",
                        f"{'[' if connection_mode == 'ipv6' else ''}{client.addr}{']' if connection_mode == 'ipv6' else ''}:{client.port}"
                        f"在ClientKeepAliveCheck中自己断开连接了，不管它",
                    )

                Thread(
                    target=lambda client=client, f=send: f(client),
                    name=f"ClientKeepAliveCheckHandler({'[' if connection_mode == 'ipv6' else ''}{client.addr}{']' if connection_mode == 'ipv6' else ''}:{client.port})",
                ).start()
            except:
...
class Client(Connection):
    ...
    def get_request(self, type: str | Iterable[str], timeout: float = -1) -> DataPack:
        """获取一个请求，并且把这个请求从请求列表中移除
    ...
    def connect(self, server_addr: str | None = None, server_port: int | None = None) -> bool:
        "连接服务器"
        ...
        try:
            Base.log(
                "I",
                f"正在连接服务器{self.server_addr}:{self.server_port}，发送ClientHello",
            ...
            Base.log(
                "I",
                f"访问服务器{self.server_addr}:{self.server_port}成功，接收到ServerHello，尝试确认连接，发送ClientConfirm",
                "Connection.Client.connect",
            )
            ...
            if req.type == SocketMsg.Connection.ServerConfirm:
                Base.log(
                    "I",
                    f"接收到ServerConfirm，连接服务器{self.server_addr}:{self.server_port}成功，服务器确认连接",
                    "Connection.Client.connect",
                )
                self.connected = True
                return True
            if req.type == SocketMsg.Connection.ServerError:
                Base.log(
                    "W",
                    f"接收到ServerError，连接服务器{self.server_addr}:{self.server_port}失败，服务器拒绝连接，详细如下",
                    "Connection.Client.connect",
                )
            ...
            Base.log_exc(
                f"连接服务器{self.server_addr}:{self.server_port}失败，发生未知错误", "Connection.Client.connect"
            )
    ...
    def wait_for_keepalive_check(self):
        ...
        while True:
            ...
            if req.type == SocketMsg.Connection.ServerKeepAliveCheck:
                Base.log(
                    "I",
                    f"接收到ServerKeepAliveCheck（内容：{req.data!r}），发送ServerKeepAliveCheckReply",
                ...
                Base.log(
                    "I",
                    f"接收到ServerDisconnect（内容：{req.data!r}），服务器要求断开连接，发送ClientDisconnect（多半是超时了）",
                    "Connection.Client.wait_for_keepalive_check",
                )
    ...
    def check_server_connection(self):
        ...
        for i in range(retry):
            ...
            try:
                ...
                Base.log(
                    "I",
                    f"收到了服务器的ClientKeepAliveCheckReply (内容：{resp.data})，连接继续",

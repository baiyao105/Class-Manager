...
def login():
    ...
    def usr_login():
        ...
        usr_name = var_usr_name.get()
        Base.log("I", f"usr_login()被调用，用户名{usr_name!r}")
        ...
        if usr_name in usrs_info:
            ...
            if usr_pwd == str(usrs_info[str(usr_name)]):
                Base.log("I", f"用户{usr_name!r}登录成功")
                ...
                Base.log("I", f"用户{usr_name!r}登录失败，密码错误")
                tkinter.messagebox.showerror(message="密码错误，请重新输入！")
        else:  # 如果发现用户名不存在
            Base.log("I", f"用户{usr_name!r}登录失败，用户不存在，尝试询问注册")

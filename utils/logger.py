...
class Logger:
    ...
    captured_stdout = SystemLogger(stdout_orig, logger_name="sys.stdout",
                                function=lambda m: Logger.log("I", m, "sys.stdout"))
    "经过处理的输出"

    captured_stderr = SystemLogger(stderr_orig, logger_name="sys.stderr",
    ...
    def reopen_log_file():
        ...
        if log_settings.log_file_path:
            ...
            Logger.log_file = open(log_settings.log_file_path, "a",
                encoding=log_settings.encoding, errors="ignore",
                buffering=1)
    ...
    if log_style == "new":
        ...
        def log(msg_type:Literal["I", "W", "E", "F", "D", "C"], msg:str, source:str="MainThread"):
            ...
            # 如果日志等级太低就不记录
            if (msg_type == "D" 
                and Logger.log_settings.log_level not in ("D"))                \
                    or (msg_type == "I" 
                        and Logger.log_settings.log_level not in ("D", "I"))            \
                    or (msg_type == "W" 
                    ...
                    or (msg_type == "F" or msg_type == "C" and 
                        Logger.log_settings.log_level not in ("D", "I", "W", "E", "F", "C")):
                return
        ...
        def log(msg_type:Literal["I", "W", "E", "F", "D", "C"], msg:str, source:str="MainThread"):
            ...
            for m in msg.splitlines():
                ...
                if file.startswith(("/", "\\")):
                    file = file[1:]
                cm = (f"{Color.BLUE}{get_time()}{Color.END} {color}{msg_type}{Color.END} "
    ...
    console_log_thread = Thread(target=lambda: Logger.log_thread_console(), # pylint: disable=unnecessary-lambda
                                daemon=True, name="ConsoleLogger")
    "把日志写在终端的线程的线程对象"

    logfile_log_thread = Thread(target=lambda: Logger.log_thread_logfile(), # pylint: disable=unnecessary-lambda
    ...
    def log_exc(info:str="未知错误：", 
        ...
        Logger.log(level, ("").join(traceback.format_exception(exc.__class__, exc, exc.__traceback__)), sender)

...
class LoggerManager:
    ...
    def setup_logger(
        ...
        **kwargs,
        ...
        if console_output:
            ...
            logger.add(
                sys.stderr, format=DEFAULT_LOG_FORMAT, level=console_level, colorize=True, backtrace=True, diagnose=True
            )
    ...
    def add_file_handler(self, file_path: str, level: str = "INFO", format_str: Optional[str] = None, **kwargs) -> None:
        """添加文件处理器
        ...
        logger.add(file_path, format=format_str, level=level, encoding="utf-8", **kwargs)

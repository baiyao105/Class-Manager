...
class DataObject:
    ...
    conn_list: Dict[str, sqlite3.Connection] = {}
    "连接列表，conn_list[数据类型名称]=连接对象"
...
class Chunk:
    ...
    def save(self, 
        ...
        """
        # TODO: 修复有的时候保存数据时间长得逆天的问题
        # 问题主要体现在大数据(167580个记录)在第一次加载时时间可能格外长，甚至能到200多秒，但是再加载一次就只要5秒
        # 试一试在建完表之后给连接断开一下，然后重新连接，速度应该会快很多

...
class DataKind(Generic[_RT], str):
    ...
    def __getattr__(self, item: str) -> Any:
        ...
        if isinstance(value, UUIDKind):
            try:
                return DataObject.loaded_object_list[
                    (self.history_uuid, self.data_type.chunk_type_name, value.uuid)
                ]
            except KeyError:
                obj = self.data_type.from_string(
                    self.chunk.get_object_rdata(
                        self.history_uuid, value.uuid, self.data_type.chunk_type_name
                    )
                )
...
class DataObject:
    ...
    loaded_object_list: Dict[
        Tuple[UUIDKind[History], str, UUIDKind[ClassDataType]], ClassDataType
    ] = {}
    "加载目标列表"
        ...
        while max_retry:
            ...
            for i in range(3):
                try:
                    ...
                    cursor.execute(
                        f"SELECT class FROM datas_{uuid[:1]} WHERE uuid = ?", (uuid,)
                    )
                    ...
                    if existing_class:
                        if existing_class[0] == type_name:
                            ...
                            raise ValueError(
                                f"对于{uuid!r}的对象，数据库中已经存在一个不同类型的对象！"
    ...
    def load_stage1(self, data: str) -> ClassDataType:
        ...
        if data_type == Student.chunk_type_name:
            self.student_history: List[UUIDKind[ScoreModification]] = data.pop(
                "history"
            )
            self.student_achievements: List[UUIDKind[Achievement]] = data.pop(
                "achievements"
            )
            data["history"] = {}
            data["achievements"] = {}
            ...
            self.homeworkrule_rule_mapping: Dict[
                str, UUIDKind[ScoreModificationTemplate]
            ] = data.pop("rule_mapping")
            data["rule_mapping"] = {}
            ...
            self.class_cleaning_mapping: List[
                Tuple[
                    int,
                    List[Tuple[Literal["member", "leader"], List[UUIDKind[Student]]]],
                ]
            ] = data.pop("cleaning_mapping")
            # 复杂数据处理逻辑
            # 其实我的意思是这个类型注释过于幽默了
            self.class_homework_rules: Dict[str, UUIDKind[HomeworkRule]] = data.pop(
                "homework_rules"
            )
            data["students"] = {}
            data["groups"] = {}
            ...
            self.achievement_template: UUIDKind[AchievementTemplate] = data.pop(
                "template"
            )
            ...
            self.atdinfo_is_late_more: List[UUIDKind[Student]] = data.pop(
                "is_late_more"
            )
            ...
            self.atdinfo_is_leave_early: List[UUIDKind[Student]] = data.pop(
                "is_leave_early"
            )
            self.atdinfo_is_leave_late: List[UUIDKind[Student]] = data.pop(
                "is_leave_late"
            )
            data["is_early"] = []
            data["is_late"] = []
            ...
            self.dayrecord_attendance_info: UUIDKind[AttendanceInfo] = data.pop(
                "attendance_info"
            )
...
class Chunk:
    ...
    database_connections: Dict[
        Tuple[Union[UUIDKind[History], Literal["Current"]], str], sqlite3.Connection
    ] = {}
    ...
    def load_history(
        self,
        history_uuid: Union[UUIDKind[History], Literal["Current"]] = "Current",
        request_uuid: Optional[UUIDKind[Type[None]]] = None,
    ) -> History:
        """
        加载历史记录。
        ...
        if "python_version" in info:
            ...
            if data_python_ver != current_ver:
                ...
                if (
                    ...
                    and request_uuid not in flags["noticed_version_changed"]
                ):
        ...
        if load_all:
            for uuid in info["histories"]:
                try:
                    ...
                    Base.log_exc(
                        f"历史记录{uuid}加载失败，将跳过", "Chunk.load_data", "E", e
                    )

...
class UISettings(BaseSettings):
    ...
    # 分数变化颜色设置
    score_up_color_begin: tuple[int, int, int] = Field(default=(0xCA, 0xFF, 0xCA), description="分数上升开始颜色")
    score_up_color_end: tuple[int, int, int] = Field(default=(0x33, 0xCF, 0x6C), description="分数上升结束颜色")
    score_down_color_begin: tuple[int, int, int] = Field(default=(0xFC, 0xB5, 0xB5), description="分数下降开始颜色")
    score_down_color_end: tuple[int, int, int] = Field(default=(0xA9, 0x00, 0x00), description="分数下降结束颜色")
...
class DatabaseSettings(BaseSettings):
    ...
    database_url: str = Field(default="sqlite:///./class_manager.db", description="数据库连接URL")
...
class BackupSettings(BaseSettings):
    ...
    auto_backup_scheme: Literal["none", "only_data", "all"] = Field(default="only_data", description="自动备份方案")
